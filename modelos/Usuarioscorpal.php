<?php
 ...
 require "../config/ConexionSQLcorpal.php";
...
Class Usuarios
 {
  ...
  public function login($usuario,$clave){
    ...
    $login ="SELECT * FROM GA_WMS_TUsuario WHERE usuario='$usuario' AND clave='$clave' AND id_empresa=2";
  ...
  //#############VALIDAR-COORDENADA--##############################
    ...
    public function validarProducto($codigo){
        ...
        $msg="";
        $und="";
        $query=ejecutarProcedureSQL("exec  GA_WMS_PGetCodigoAlterno ?,'GPCAL',0,?,?");
    ...
    public function inforArtiCoor($codigo,$coordenada){
        $sql="exec  GA_WMS_PGetAsignadoUbicacion '$coordenada','$codigo','GPCAL'";
        return ejecutarConsultaSQL($sql);
    }

    public function movimientos($coorInicio,$coorFin,$cantidad,$producto,$usuario,$tipo){
        $msg="";
        $query=ejecutarProcedureSQL("exec GA_WMS_PInsmovimiento ?,?,?,?,?,'GPCAL',?,'',?");
