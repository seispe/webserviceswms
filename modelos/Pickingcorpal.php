...
Class Picking
 {
  ...
  //#############CONSOLIDADIOS-##############################
    ...
    public function obtenerProductosConsolidado($consolidado,$usuario){
        $msg="";
        $query=ejecutarProcedureSQL("exec GA_WMS_PGetRutaPicking '$consolidado','GPCAL','00','$usuario',?");
    ...
    public function guardarPicking($consolidado,$producto,$origen,$solicitada,$procesada,$pendiente,$destino,$usuario){
        ...
        $msg="";
        $query=ejecutarProcedureSQL("exec [GA_WMS_PInspickingconsolidado] '$consolidado', '$producto', '$origen', '$solicitada', '$procesada', '$pendiente', '$destino', '$usuario','GPCAL','' ,? ");
        $query->bindParam(1, $msg, PDO::PARAM_STR | PDO::PARAM_INPUT_OUTPUT, 50);
        $query->execute();
        if (strlen($msg) > 0 ) {
            return $msg;
